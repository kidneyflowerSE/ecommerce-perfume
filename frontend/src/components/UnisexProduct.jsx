...
const UnisexProduct = ({ filteredProducts }) => {
  ...
  useEffect(() => {
    ...
    const storedProducts = localStorage.getItem("unisexProducts");

    if (storedProducts) {
      setUnisexProducts(JSON.parse(storedProducts));
      setLoading(false);
    } else {
      axios
        .get("http://localhost:8080/api/products/category/Nước%20hoa%20unisex")
        .then((response) => {
          setUnisexProducts(response.data);  // Fix here
          localStorage.setItem("unisexProducts", JSON.stringify(response.data)); 
          setLoading(false);
        })
        .catch((error) => {
  ...
  if (loading) return <div className="w-full h-full"><LoadingPage /></div>;
  if (error) return <div>Lỗi: {error}</div>;

  const productsToDisplay = filteredProducts?.length > 0 ? filteredProducts : unisexProducts;
